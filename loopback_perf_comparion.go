// Loopback interface performance comparision between using applying a bpf filter(using pacp) vs standard socket.
package main
...
//
// func main() {
// 	defaultDevice := "en0" // Use the physical network interface (e.g., en0)
//
// 	// Benchmark PCAP/BPF
// 	fmt.Println("Running pcap/BPF benchmark...")
// 	donePCAP := make(chan bool)
// 	go generateUDPTraffic(donePCAP, "192.168.1.100") // Replace with the target machine's IP
// 	time.Sleep(100 * time.Millisecond)               // Let traffic start
// 	benchmarkPcap(defaultDevice)
// 	donePCAP <- true
//
...
// 	fmt.Println("\nRunning standard socket benchmark...")
// 	doneSocket := make(chan bool)
// 	go generateUDPTraffic(doneSocket, "192.168.1.100") // Replace with the target machine's IP
// 	time.Sleep(100 * time.Millisecond)
// 	benchmarkStandardSocket()
// 	doneSocket <- true
// }
